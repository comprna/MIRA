...
def main(argv):
	...
	'''
	#read fasta sequence
	genome_path = '/projects_rg/babita/ref_genome/hg19.fa'
	bed_file = '/projects_rg/babita/TCGA/mutation/mut_pipeline/testfiles/Refseq_genes_hg19_new.bed'
	tmp_file = '/projects_rg/babita/TCGA/mutation/mut_pipeline/testfiles/tmp'
	kmer_len = 10
	mutation_bed_file = '/projects_rg/TCGA_Fred/tsv/mutations_v2_formatted.tsv'
	output_gene_lib_dir = '/projects_rg/babita/TCGA/mutation/mut_pipeline/testfiles/gene_lib'
	final_outfile = '/projects_rg/babita/TCGA/mutation/mut_pipeline/testfiles/test_final_score.out'
	'''
	...
	'''
	IMPORTANT: Before running this script create mutation_table file "*.mut.out"
	example:
	...
	input_all_mutation_file = sys.argv[3]
	output_gene_lib_dir = sys.argv[4]
	input_gene_file = sys.argv[5]
	line = sys.argv[5]

	check1 = check_files(input_all_mutation_file)
	check2 = check_files(input_gene_file)

	if len(sys.argv) == 6 and (check1 and check2):
		...
		for line in rf:
			...
			if check:
				...
				check2 = create_library.create_gene_library(gname, input_all_mutation_file, output_gene_lib_dir)
			...
			if(check2):
				...
				if(check):				
					check = overlap_kmers(mutation_bed_file, tmp_kmers_bed_file3, tmp_overlap_bed_file4)
				...
				if(check):
					...
					#print("Done step : 5")
					check = statistics(line, kmeroutfile5, kmer_len, total_gene_mut, 
					...
					if(check):
						...
						print("No mutation found on gene kmers; condtion >2 mutations not true for gene: {}".format(gname))
		...
		print("Script Usage:\npython run_pipeline_clusters.py <1.kmer_len> <2.genome.fa> <3.all_mutation_file> <4.out_dir> <5.input_gene.bed>")
...
def overlap_kmers(mutation_file, tmp_kmers_file, tmp_overlap_file):
	'''
	...
	os.system("/soft/bio/sequence/bedtools-2.25.0/bin/intersectBed -wo -a {} -b {} >{}".format(
		tmp_kmers_file, mutation_file, tmp_overlap_file))
...
def statistics(line, tmp_overlap_file, N, mut_on_gene, genome_path, mutation_bed_file, outfile):
	'''
	Test1:
	...
	while i <= int(stop):   #for each kmer window of len N
		...
		#if mut_on_kmer > 2: #(2 is arbitrary cut-off for number of mutations on the given Kmer is 3 or more)
		
		if mut_on_kmer >= 1: #testing purpose for qqplot

		###if mut_on_kmer == 2: #cut-off for test purpose only
 			'''
